...
import { formPath, DEFAULT_FORM_SLUG, type ArtistWithCategory, type CtaBlock } from './lib/database.types';
    ...
    <div ref={cardRef} className="group cursor-pointer" onClick={onClick}>
      ...
      <div className="mt-5 space-y-2">
        ...
        {artist.artist_categories?.name && (
          <p className="text-xs font-mono text-primary/60 tracking-wider uppercase">
            {artist.artist_categories.name}
          </p>
        )}
        ...
        {artist.bio && (
          <p className="text-sm text-primary/60 leading-relaxed line-clamp-2">{artist.bio}</p>
        )}
      </div>
    </div>
  );
        ...
        <section className="relative min-h-[60vh] flex flex-col items-center justify-center px-6 md:px-20 py-20 md:py-32 border-b border-primary/20 overflow-hidden">
          ...
          <div className="relative z-10 w-full max-w-7xl">
            ...
            <EditableText
              ...
              field="title"
              value={heroTitle}
              onSave={v => setArtistsContent('title', v)}
          ...
          <div className="max-w-7xl mx-auto space-y-8">
            ...
            {loading ? (
              ...
              <div className="text-center py-20">
                <p className="text-primary/50 text-xl font-mono tracking-widest">ERROR LOADING ARTISTS</p>
                <p className="text-primary/30 text-sm font-mono mt-3">Something went wrong loading this. Please try again in a moment.</p>
              </div>
            ) : filteredArtists.length === 0 ? (
              <div className="text-center py-20">
                <p className="text-primary/50 text-xl font-mono tracking-widest">{artists.length === 0 ? 'NO ARTISTS YET' : 'NO RESULTS'}</p>
                <p className="text-primary/30 text-sm font-mono mt-3">{artists.length === 0 ? 'Check back soon.' : 'Try a different filter.'}</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 sm:gap-10 md:gap-12">
                {filteredArtists.map((artist, index) => (
                  <ArtistCard key={artist.id} artist={artist} index={index} onClick={() => handleNav(`/artists/${artist.slug}`)} />
                ))}
              </div>
            )}
          </div>
