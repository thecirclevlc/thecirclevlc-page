    ...
    <div ref={sectionRef} className="relative h-screen overflow-hidden">
      ...
      <div ref={trackRef} className="flex items-center h-full gap-4 md:gap-6 pl-6 md:pl-20 will-change-transform">
        {images.map((img, i) => (
          <div
            key={i}
            className="flex-shrink-0 relative overflow-hidden border border-primary/20 cursor-pointer group"
...
const NameRow: React.FC<{
  ...
  <div
    className="group cursor-pointer flex items-baseline justify-between gap-3 py-3 border-b border-primary/10 hover:border-primary/40 transition-colors duration-200"
    onMouseEnter={() => { if (photo_url) onSetHover(photo_url); }}
    onMouseLeave={() => onSetHover(null)}
    onClick={onClick}
  >
    <span className="text-xl md:text-2xl font-black text-primary uppercase tracking-tight leading-none group-hover:text-white transition-colors duration-200">
      {name}
    </span>
    {subtitle && (
      <span className="text-[10px] font-mono text-primary/40 tracking-wider whitespace-nowrap flex-shrink-0">
        {subtitle}
      </span>
    )}
  </div>
...
export default function EventDetail() {
  ...
  // Después de declarar `event`: las cuatro fichas ponían literalmente
  // "Event" en la pestaña del navegador.
      ...
      <div className="relative z-10 pt-16 md:pt-20">
        ...
        <section ref={heroRef} className="relative h-[100dvh] flex items-end overflow-hidden">
          ...
          <div className="relative z-10 w-full px-6 md:px-20 pb-20 md:pb-32">
            {/* Breadcrumb */}
            <div className="flex items-center gap-2 text-xs font-mono text-primary/40 tracking-wider mb-6">
              <button
              ...
              <span>/</span>
              <span className="text-primary/70 truncate max-w-[200px]">{event.title}</span>
            </div>

            {event.event_number && (
              <div className="hero-number text-primary/40 font-black text-[15vw] md:text-[10vw] leading-none mb-4">
                {event.event_number}
              </div>
            )}
            ...
            {/* The admin has always had a ticket link field; nothing ever
                rendered it, so every URL she typed went nowhere. */}
            {event.ticket_url && (
              <a
        ...
        <section className="relative px-6 md:px-20 py-16 md:py-24 border-t border-primary/20">
          <div className="max-w-7xl mx-auto">
            <div className={`grid grid-cols-1 gap-12 md:gap-0 ${
                ...
                {event.description && (
                  <div className="space-y-4">
                    {event.description.split('\n').filter(Boolean).map((para, i) => (
                      <p key={i} className="text-sm text-primary/60 leading-relaxed">{para}</p>
                    ))}
                  </div>
                )}
              ...
              {(eventDJs.length > 0 || showLegacyLineup) && (
                <div className="md:px-10 md:border-l md:border-primary/10 space-y-4">
                  <p className="text-[10px] font-mono text-primary/40 tracking-[0.2em] uppercase">DJs — Line Up</p>
                  <div>
                    {eventDJs.map(dj => (
                      <NameRow
                        key={dj.id}
                    ...
                    {showLegacyLineup && (event.lineup ?? []).map((name, i) => (
                      <div key={i} className="py-3 border-b border-primary/10 text-xl font-black text-primary uppercase tracking-tight">
                        {name}
                      </div>
                    ))}
                  </div>
                  <button
                    ...
                    className="mt-4 text-xs font-mono text-primary/40 hover:text-primary tracking-widest uppercase transition-colors cursor-pointer"
                  >
                    View all DJs &rarr;
                  </button>
                </div>
              ...
              <div className="flex flex-wrap gap-6 items-center">
                {(event.partnerships as any[]).map((p: any, i: number) => (
                  <a
                    ...
                    rel="noopener noreferrer"
                    className="flex items-center gap-3 px-5 py-3 border border-primary/20 hover:border-primary/40 transition-colors group"
                  >
                    {p.logo_url && (
                      <img src={p.logo_url} alt={p.name} className="h-8 w-auto object-contain opacity-70 group-hover:opacity-100 transition-opacity" />
                    )}
                    <span className="text-sm font-mono text-primary/70 group-hover:text-primary tracking-wider uppercase transition-colors">
                      {p.name}
                    </span>
                  </a>
                ))}
        ...
        {hoveredUrl && (
          <div className="fixed right-10 top-1/2 -translate-y-1/2 pointer-events-none z-[200]
                          w-48 h-60 overflow-hidden border border-primary/30 shadow-2xl
                          hidden md:block">
            <img
              src={hoveredUrl}
              alt=""
            ...
            >
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
                ...
                {nextEvent.event_number && (
                  <div className="text-lg md:text-xl font-light text-primary/80 mb-4">{nextEvent.event_number}</div>
                )}
                {nextEvent.date && (
                  <div className="text-sm font-mono text-primary/50 mb-8">
                    {formatDate(nextEvent.date)}
                  </div>
                )}
                <span className="inline-block text-sm font-mono tracking-widest text-primary border border-primary/40 px-6 py-3 group-hover:bg-primary group-hover:text-black transition-all duration-300 uppercase">
                  View Event &rarr;
                </span>
              </div>
            </div>
